...
class CalendarHeatmap extends React.Component {
  ...
  parseData () {
    ...
    this.selected = this.props.data[0];
  }
  ...
  drawGlobalOverview () {
    ...
    let item_block = this.items
      ...
      .on ('mouseover', d => {
        ...
        // Construct tooltip
        let tooltip_html = '';
        tooltip_html += `<div><span><strong>Total crimes: ${d.total}</strong></span>`;
  ...
  drawYearOverview () {
    ...
    this.items
      ...
      .on ('mouseover', d => {
        ...
        // Construct tooltip
        let tooltip_html = '';
        tooltip_html += `<div class="${styles.header}"><strong>${d.total} crimes</strong></div>`;
        tooltip_html +=
          '<div>on ' +
  ...
  drawMonthOverview () {
    ...
    let month_data = this.props.data.filter (d => {
      return (
        start_of_month <= moment (d.date) && moment (d.date) < end_of_month
      );
    });
    ...
    item_block
      ...
      .attr ('x', function (d) {
        ...
        d3
          ...
          .attr ('offset', offset + itemScale (d.value));
        return offset;
      })
      ...
      .on ('mouseover', d => {
        ...
        // Construct tooltip
        let tooltip_html = '';
        tooltip_html += `<div class="${styles.header}"><strong>${d.value} crimes</strong></div><br>`;
        tooltip_html +=
          '<div>on ' + moment (date).format ('dddd, MMM Do YYYY') + '</div>';

        // Calculate tooltip position
        let x =
          weekScale (moment (date).week ()) + this.settings.tooltip_padding;
        while (
          this.settings.width - x <
          this.settings.tooltip_width + this.settings.tooltip_padding * 3
        ) {
